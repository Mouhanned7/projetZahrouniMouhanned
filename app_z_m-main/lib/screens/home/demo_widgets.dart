...
class _PP extends CustomPainter {
  ...
  void paint(Canvas c, Size s) {
    for (final p in ps) {
      ...
      c.drawCircle(Offset(dx, dy), p.rad, Paint()..color = p.col.withOpacity((0.15 + 0.2 * sin(t * 2 * pi + p.ph)).clamp(0.05, 0.35)));
    }
  }
  @override
...
class GlassCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(padding: padding,
    decoration: BoxDecoration(
      gradient: LinearGradient(colors: [Colors.white.withOpacity(0.1), Colors.white.withOpacity(0.03)], begin: Alignment.topLeft, end: Alignment.bottomRight),
      borderRadius: BorderRadius.circular(radius),
      border: Border.all(color: Colors.white.withOpacity(0.12)),
      boxShadow: [BoxShadow(color: (glow ?? Colors.black).withOpacity(0.15), blurRadius: 30, offset: const Offset(0, 12))],
    ), child: child);
}
...
class _H3State extends State<Hover3DCard> {
  ...
  Widget build(BuildContext c) => MouseRegion(
    ...
    child: AnimatedContainer(duration: const Duration(milliseconds: 200),
      transform: Matrix4.identity()..setEntry(3, 2, 0.001)..rotateX(_rx)..rotateY(_ry)..scale(_h ? 1.02 : 1.0),
